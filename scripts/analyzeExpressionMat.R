...
.write_sparse <-
  function(path,
    ...
    gene.info <-
      data.frame(gene.id, gene.symbol, stringsAsFactors = FALSE)
    
    gene.info$gene.type <-
      rep(gene.type, length.out = nrow(gene.info))
    mhandle <- file.path(path, "matrix.mtx")
    bhandle <-
      gzfile(file.path(path, "barcodes.tsv.gz"), open = "wb")
    fhandle <-
      gzfile(file.path(path, "features.tsv.gz"), open = "wb")
    on.exit({
      close(bhandle)
      close(fhandle)
    })
...
FindDiffExprFeatures <-
  function(geneFile,
    ...
    TARMat <-
      as.sparse(TARMat) #force to sparse to match with gene matrix
    ...
    npcs_significant <- .find_npcs(combined.seu,
                                   var.toal = 0.95,
                                   reduction = "pca_genes")
    
    combined.seu <- FindNeighbors(combined.seu,
                                  reduction = "pca_genes",
                                  dims = 1:npcs_significant)
    
    combined.seu <- FindClusters(combined.seu,
    ...
    markersUTAR <-
      markersUTAR[order(markersUTAR$avg_log2FC, decreasing = T), ]
    ...
    markersGenes <-
      markersGenes[order(markersGenes$avg_log2FC, decreasing = T), ]
...
if (dirname(TARFile) != ".") {
  ...
  output <-
    paste0(dirname(TARFile), "/", "diff_Expressed_Features.txt")
} else{
